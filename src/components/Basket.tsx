    ...
    >
      <div className="basketContainer" onClick={(e) => e.stopPropagation()}>
        <div className="basketHeader">
          ...
          {" ~" +
            props.listOfBasket
              .reduce(
                (sum, item) =>
                  item.count > 0 ? sum + item.weight * item.count : sum,
                0
              )
              .toFixed(2) +
            "кг"}
        ...
        <div className="basketMain">
          {props.listOfBasket.find(
            (item) => item.count !== undefined && item.count > 0
          ) ? (
            <FullBasket
            ...
            <EmptyBasket setBasketFlag={props.setBasketFlag} />
          )}
        </div>
        <div className="basketFooter">
          {props.listOfBasket.find(
            ...
            <div className="registrationContainer">
              <div className="registrationPrice">
                Цена заказа:{" "}
                {props.listOfBasket
                  .reduce(
                    (sum, item) =>
                      item.count > 0 ? sum + item.price * item.count : sum,
                    0
                  )
                  .toFixed(0) + "р"}
              </div>
          ...
          product.count && product.count > 0 ? (
            <div key={product.id} className="productBasketContainer">
              <div className="imgBasketContainer">
                <div
                  className="imgInBasket"
                  style={{ backgroundImage: `url(${product.img})` }}
                />
                <p className="nameProductInBasket">
              ...
              <div style={{ width: "30%" }}>
                {product.price * product.count +
                  "р / " +
                  (product.weight * product.count).toFixed(2) +
                  product.unitOfMass}
              </div>
